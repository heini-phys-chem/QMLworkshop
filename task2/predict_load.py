...
#    model.train(train_coordinates, train_charges, train_energies, train_forces if args.forces else None)
    model = torch.jit.load('/home/heinen/QMLworkshop/task2/model_sorf_2k.pt')
    ...
    RMSE_e = np.array([])
    RMSE_f = np.array([])
    for i in tqdm.tqdm(range(test_energies.shape[0])):
    #    for i in range(test_energies.shape[0]):
        if (args.forces):
            xyz = torch.tensor(test_coordinates[i], device='cuda', dtype=torch.float32, requires_grad=True)
            ...
            energy_predictions = model.forward(xyz[None], charges[None], atomIDs, molIDs, atom_counts)
            forces_torch, = torch.autograd.grad(-energy_predictions.sum(), xyz)
        else:
        ...
        force_predictions  = forces_torch.cpu().detach().numpy()
