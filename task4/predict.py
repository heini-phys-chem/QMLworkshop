...
#    train_indexes = train_IDs[:ntrain]
    ...
    train_coordinates = coords_train
    train_charges     = nuclear_charges_train
    train_energies    = energies_train
    train_forces      = forces_train
    ...
    if (args.hyperparam_opt):
        model.hyperparam_opt_nested_cv(train_coordinates, train_charges, train_energies, sigmas=np.array([4.8, 9.6, 19.2, 38.4]), lambdas=np.array([1e-5, 1e-7, 1e-9]), F=train_forces if args.forces else None)
    ...
    max_natoms = 384 #data['natom_counts'].max().item()
    ...
    #        print(force_predictions)
        e_pred = np.append(e_pred, energy_predictions)
        f_pred = np.append(f_pred, force_predictions)
    ...
    np.savez("task4_energy_200.npz", e_pred)
    np.savez("task4_forces_200.npz", f_pred)
