...
#    train_indexes = train_IDs[:ntrain]
    ...
    if (args.hyperparam_opt):
        model.hyperparam_opt_nested_cv(train_coordinates, train_charges, train_energies, sigmas=np.array([9.6, 19.2]), lambdas=np.array([1e-9]), F=train_forces if args.forces else None)

    #model.train(train_coordinates, train_charges, train_energies, cell=cell_tensor, cell_inv=cell_inv, train_forces if args.forces else None)
    ...
    max_natoms = 114 #data['natom_counts'].max().item()
