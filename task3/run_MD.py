...
model  = torch.jit.load('/home/heinen/QMLworkshop/task3/model_sorf.pt')
initial_atoms = read('test.xyz')
# Set the unit cell and enable PBC
initial_atoms.set_cell(cell)  # Set the cell and scale the positions accordingly
initial_atoms.set_pbc(True)  # Enable PBC
...
total_steps = 200  # Total number of MD steps
...
#for i in range(len(temps)):
    #dyn = Langevin(initial_atoms, timestep=time_step * units.fs, temperature_K=temps[i], friction=friction_coefficient)
...
#for step in range(steps[i]):
    ...
    if step % 10 == 0:
        ...
        #filename = f'xyz/coords_{temps[i]:03d}_{step:05d}.xyz'
        filename = f'xyz/coords_{step:05d}.xyz'

        # Write the coordinates to the XYZ file
        write(filename, initial_atoms, format='xyz')
        #print("Temp: {}, Step: {}, E: {}".format(temps[i], step, initial_atoms.get_potential_energy()))
