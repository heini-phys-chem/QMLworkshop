...
#    data = np.load(args.path, allow_pickle=True)
    data_train = np.load("600/train.npz", allow_pickle=True)
    data_val   = np.load("600/validation.npz", allow_pickle=True)
...
#    train_indexes = train_IDs[:ntrain]
    ...
    max_natoms = 42 #data['natom_counts'].max().item()
    ...
    print("Energy MAE /w backwards:", np.mean(np.abs(energy_predictions - test_energies)))

    if (args.forces):
        #print("Force MAE /w backwards:", torch.mean(torch.abs(force_predictions - test_forces)))
        print("Force MAE /w backwards:", np.mean(np.abs(force_predictions - test_forces)))
